...
from app.feature.baseline_weight.baseline_weight_controller import (
    BaselineWeightController,
)
from app.feature.employee_request.employee_request_controller import (
...
from app.feature.organization_employee.organization_employee_controller import (
    OrganizationEmployeeController,
)
from app.feature.organization_type.organization_type_controller import (
    OrganizationTypeController,
)
from app.feature.payment_document.payment_document_controller import (
...
from app.feature.schedule_history.schedule_history_controller import (
...
from app.feature.vehicle_category.vehicle_category_controller import (
...
from app.feature.workshop_schedule.workshop_schedule_controller import (
    WorkshopScheduleController,
)
...
def include_routers(app) -> None:
    ...
    app.include_router(
        UserTypeController().router, prefix="/user-type", tags=["user-type"]
    )
    app.include_router(UserController().router, prefix="/user", tags=["user"])
    app.include_router(
        OrganizationTypeController().router,
        prefix="/organization-type",
        tags=["organization-type"],
    )
    app.include_router(
        OrganizationController().router, prefix="/organization", tags=["organization"]
    )
    app.include_router(
        OrganizationEmployeeController().router,
        prefix="/organization-employee",
        tags=["organization-employee"],
    )
    app.include_router(
        VehicleColorController().router, prefix="/vehicle-color", tags=["vehicle-color"]
    )
    app.include_router(
        VehicleCategoryController().router,
    ...
    app.include_router(
        OrderStatusController().router, prefix="/order-status", tags=["order-status"]
    )
    app.include_router(OrderController().router, prefix="/order", tags=["order"])
    app.include_router(
        SapRequestController().router, prefix="/sap-request", tags=["sap-request"]
    )
    app.include_router(KaspiPaymentController().router, prefix="/kaspi", tags=["kaspi"])
    app.include_router(
        OperationController().router, prefix="/operation", tags=["operation"]
    )
    app.include_router(
        WorkshopScheduleController().router,
        prefix="/workshop-schedule",
        tags=["workshop-schedule"],
    )
    app.include_router(ScheduleController().router, prefix="/schedule", tags=["schedule"])
    app.include_router(
        ScheduleHistoryController().router,
        prefix="/schedule-history",
        tags=["schedule-history"],
    )
    app.include_router(
        ActWeightController().router, prefix="/act-weight", tags=["act-weight"]
    )
    app.include_router(
        InitialWeightController().router,
        prefix="/initial-weight",
        tags=["initial-weight"],
    )
    app.include_router(
        BaselineWeightController().router,
        prefix="/baseline-weight",
        tags=["baseline-weight"],
    )
    app.include_router(
        EmployeeRequestController().router,
        prefix="/employee-request",
        tags=["employee-request"],
    )
    app.include_router(
        PaymentDocumentController().router,
        prefix="/payment_document",
        tags=["Upload Payment File"],
    )
