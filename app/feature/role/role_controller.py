...
class RoleController:
    ...
    def _add_routes(self):
        self.router.get("/", response_model=List[RoleRDTO], summary="Список ролей",
                        description="Получение списка ролей")(self.get_all)
        self.router.post("/create", response_model=RoleRDTO, summary="Создать роль", description="Создание роли")(
            self.create)
        self.router.get("/get_by_id/{id}", response_model=RoleRDTO, summary="Получить роль по уникальному ID",
                        description="Получение роли по уникальному идентификатору")(self.get_by_id)
        self.router.put("/update/{id}", response_model=RoleRDTO, summary="Обновить роль по уникальному идентификатору",
                        description="Обновление одной роли")(self.update)
        self.router.delete("/delete/{id}", summary="Удалить роль по уникальному идентификатору",
                           description="Удаление одной роли")(self.delete)
