    ...
    async def request_to_sap(
        ...
        try:
            ...
            if(order.organization_id is not None):
                data = await sapService.request_to_sap_legal(werks=sap_request.werks,matnr=sap_request.matnr,dogovor=sap_request.dogovor)
                ...
                sap_request_dto.pdf = base64.b64decode(pdf_base64) if pdf_base64 else None  # Декодирование Base64 в байты
                sap_request_dto.date = datetime.strptime(data.get("DATE", ""), "%Y-%m-%d").date() if data.get("DATE") else None
                sap_request_dto.time = datetime.strptime(data.get("TIME", ""), "%H:%M:%S").time() if data.get("TIME") else None
            if(order.owner_id is not None):
                ...
                pdf_base64 = data.get("PDF", "")
                sap_request_dto.pdf = base64.b64decode(pdf_base64) if pdf_base64 else None  # Декодирование Base64 в байты
                sap_request_dto.date = datetime.strptime(data.get("DATE", ""), "%Y-%m-%d").date() if data.get(
                    "DATE") else None
                sap_request_dto.time = datetime.strptime(data.get("TIME", ""), "%H:%M:%S").time() if data.get(
                    "TIME") else None
