...
class InitialWeightFilter(BaseFilter):
    def __init__(
        self,
        per_page: int = Query(
        ...
        vehicle_id: int | None = Query(
            default=None, description="Идентификатор транспорта"
        ),
        trailer_id: int | None = Query(default=None, description="Идентификатор прицепа"),
        history_id: int | None = Query(default=None, description="Идентификатор истории"),
        order_id: int | None = Query(default=None, description="Идентификатор заказа"),
        start_at: date | None = Query(
            default=None, description="Период с", lt=date.today()
        ),
        end_at: date | None = Query(
            default=None, description="Период до", le=date.today()
        ),
    ) -> None:
    ...
    def apply(self) -> list:
        ...
        if self.history_id:
            filters.append(and_(self.model.history_id == self.history_id))
        if self.order_id:
            filters.append(and_(self.model.order_id == self.order_id))
        if self.vehicle_id:
            filters.append(and_(self.model.vehicle_id == self.vehicle_id))
        if self.trailer_id:
            filters.append(and_(self.model.trailer_id == self.trailer_id))
