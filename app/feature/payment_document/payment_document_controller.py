...
from app.core.auth_core import check_client, check_employee
...
from app.feature.payment_document.payment_document_repository import (
...
class PaymentDocumentController:
    ...
    def _add_routes(self) -> None:
        self.router.post(
            "/upload-payment-file", summary="Прикрепление документов для проверки"
        )(self.upload_payment_doc)
        self.router.post("/get-payment-docs", summary="Получение всех документов")(
    ...
    async def upload_payment_doc(
        ...
        for document in documents:
            result = await repo.upload_file(
                userRepo=userRepo, order_id=order_id, file=document, orderRepo=orderRepo
            )
    ...
    async def get_payment_docs(
        ...
        return await repo.get_payment_docs(
            orderRepo=orderRepo, params=params, userRepo=userRepo
        )

    async def get_payment_doc_by_order(
        self,
    ...
    async def add_to_comment_to_doc(
        self,
        payment_id: int = Form(..., description="Идентификатор документа"),
        status: bool = Form(..., description="Статус документа"),
        comment: str | None = Form(None, description="Комментарии к документу"),
        repo: PaymentDocumentRepository = Depends(PaymentDocumentRepository),
        userRepo: UserRDTOWithRelations = Depends(check_employee),
    ):
        return await repo.add_comment_to_doc(
            payment_id=payment_id, status=status, comment=comment, userRepo=userRepo
        )
    ...
    async def make_decision(
        ...
        return await repo.make_decision(
            order_id=order_id, orderRepo=orderRepo, status=status, userRepo=userRepo
        )
