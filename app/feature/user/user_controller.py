...
from app.core.auth_core import (
    check_admin,
    check_admin_and_employee,
    get_password_hash,
)
from app.core.pagination_dto import PaginationUserRDTOWithRelations
from app.core.validation_rules import EMAIL_REGEX, PHONE_REGEX, TWELVE_DIGITS_REGEX
...
class UserController:
    ...
    async def get_by_iin(
        ...
        user_iin = await repo.get_filtered(
            {"iin": iin},
            options=[
                selectinload(UserModel.role),
                selectinload(UserModel.user_type),
    ...
    async def get_by_email(
        ...
        user_email = await repo.get_filtered(
            {"email": email},
            options=[
                selectinload(UserModel.role),
                selectinload(UserModel.user_type),
    ...
    async def get_by_phone(
        ...
        user_phone = await repo.get_filtered(
            {"phone": phone},
            options=[
                selectinload(UserModel.role),
                selectinload(UserModel.user_type),
    ...
    async def check_form(
        ...
        if user_iin is not None and user_iin.id != id:
