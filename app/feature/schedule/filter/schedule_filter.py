...
class ScheduleClientScheduledFilter(BaseFilter):
    def __init__(self,
                 per_page: int = Query(default=20, gt=0, example=20, description="Количество элементов на страницу"),
                 page: int = Query(default=1, ge=1, example=1, description="Номер страницы"),
                 search: Optional[str] = Query(default=None, max_length=255, min_length=3),
...
class ScheduleClientFromToFilter(BaseFilter):
    def __init__(self,
                 per_page: int = Query(default=20, gt=0, example=20, description="Количество элементов на страницу"),
                 page: int = Query(default=1, ge=1, example=1, description="Номер страницы"),
                 search: Optional[str] = Query(default=None, max_length=255, min_length=3),
                 start_at: date = Query(default=None, description="Начать искать с"),
                 end_at: date = Query(default=None, description="Начать искать до"),
    ...
    def apply(self, userRDTO: UserRDTOWithRelations) -> list:
        ...
        while current_date <= end_date:
            date_list.append((current_date,datetime.combine(date(current_date.year,current_date.month,current_date.day),time(23,59,59))))
