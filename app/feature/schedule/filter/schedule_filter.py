...
class ScheduleFilter(BaseFilter):
    def __init__(
        ...
        start_at: date | None = Query(default=None, description="Начать искать с"),
        end_at: date | None = Query(default=None, description="Начать искать до"),
        is_active: bool | None = Query(default=None, description="Заказ активен?"),
        is_used: bool | None = Query(
            default=None, description="Находится ли он на территории?"
        ),
        is_canceled: bool | None = Query(default=None, description="Отменен ли?"),
        is_executed: bool | None = Query(
            default=None, description="Успешно ли завершен?"
        ),
    ) -> None:
    ...
    def apply(self) -> list:
        ...
        if self.is_executed is not None:
...
class ScheduleClientScheduledFilter:
    def __init__(
        ...
        order_id: int | None = Query(default=None, description="Идентификатор заказа"),
    ) -> None:
    ...
    def apply(self, userRDTO: UserRDTOWithRelations) -> list:
        ...
        filters.append(
            and_(self.model.start_at <= end_at, self.model.start_at >= start_at)
        )
        if userRDTO.user_type.value == TableConstantsNames.UserLegalTypeValue:
            ...
            filters.append(
                and_(
                    or_(
                        self.model.owner_id == userRDTO.id,
                        self.model.driver_id == userRDTO.id,
                    )
                )
            )
...
class ScheduleClientFromToFilter:
    ...
    def apply(self) -> list:
        if self.start_at >= self.end_at:
            msg = "Дата начала должна быть больше даты конца"
