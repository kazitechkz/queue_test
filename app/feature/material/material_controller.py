...
from app.feature.material.dtos.material_dto import (
    MaterialCDTO,
    MaterialRDTO,
...
class MaterialController:
    ...
    async def all(
        ...
        result = await repo.get_all_with_filter(
            options=[selectinload(repo.model.workshop)]
        )
        result_dto = [
            MaterialWithRelationsDTO.from_orm(resultItem) for resultItem in result
        ]
        return result_dto
    ...
    async def get_by_sap(
        ...
        result = await repo.get_filtered(
            filters={"sap_id": sap_id}, options=[selectinload(repo.model.workshop)]
        )
        if result is None:
            raise AppExceptionResponse.not_found(message="Материал не найден")
        result_dto = MaterialWithRelationsDTO.from_orm(result)
    ...
    async def create(
        ...
        if existed is not None:
            raise AppExceptionResponse.bad_request(
                message="Материал с таким sap_id уже существует"
            )
