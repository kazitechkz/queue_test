...
from app.feature.order_status.dtos.order_status_dto import (
    OrderStatusWithRelationRDTO,
)
...
class OrderStatusController:
    ...
    async def get_all(
        ...
        result = await repo.get_all_with_filter(
            options=[
                selectinload(repo.model.prev_status),
                selectinload(repo.model.next_status),
            ]
        )
        result_dto = [
            OrderStatusWithRelationRDTO.from_orm(resultItem) for resultItem in result
        ]
        return result_dto
