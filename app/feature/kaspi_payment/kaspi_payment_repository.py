...
from app.feature.kaspi_payment.dtos.kaspi_payment_dto import (
    KaspiPaymentCDTO,
    KaspiPaymentCheckResponseDTO,
    KaspiPaymentPayResponseDTO,
)
from app.feature.kaspi_payment.query_parameters.kaspi_payment_check_param import (
    KaspiPaymentCheckParam,
)
from app.feature.kaspi_payment.query_parameters.kaspi_payment_pay_param import (
    KaspiPaymentPayParam,
)
from app.feature.order.dtos.order_dto import OrderCDTO
from app.feature.order.order_repository import OrderRepository
from app.shared.kaspi_response_codes import KaspiPaymentCodeStatus, KaspiPaymentStatus
...
class KaspiPaymentRepository(BaseRepository[KaspiPaymentModel]):
    ...
    async def check(
        self, kaspiParams: KaspiPaymentCheckParam, orderRepo: OrderRepository
    ):
        try:
            order = await orderRepo.get_first_with_filters(
                filters=[{"zakaz": kaspiParams.account}]
            )
            if order is None:
            ...
            await self.filter_and_update(filters=filters, update_values=update_values)

            kaspi_payment_dto = KaspiPaymentCDTO(
                order_id=order.id,
            ...
            kaspi_payment = await self.create(
                KaspiPaymentModel(**kaspi_payment_dto.dict())
            )
            return KaspiPaymentCheckResponseDTO(
                txn_id=kaspi_payment.txn_id,
                sum=kaspi_payment.sum,
    ...
    async def pay(
        ...
        check_payment_dto.paid_at = datetime.strptime(
            kaspiParams.txn_date, "%Y%m%d%H%M%S"
        )
