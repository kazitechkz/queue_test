...
class KaspiPaymentRepository(BaseRepository[KaspiPaymentModel]):
    ...
    async def check(
        ...
        try:
            ...
            if order is None:
                return KaspiPaymentCheckResponseDTO(txn_id=kaspiParams.txn_id,sum = 0,result=KaspiPaymentStatus.NOT_FOUND, comment="Заказ не найден")
            if order.is_paid and order.txn_id:
                return KaspiPaymentCheckResponseDTO(txn_id=kaspiParams.txn_id, sum=order.price_with_taxes, result=KaspiPaymentStatus.ALREADY_PAID,
                                                    comment="Заказ уже оплачен")

            update_values = {"is_failed": True}  # Fields to update
            filters = [{"account": kaspiParams.account}, {"command": KaspiPaymentCodeStatus.CHECK},{"is_failed":False}]
    ...
    async def pay(
            ...
            if order.is_paid and order.txn_id:
                return KaspiPaymentPayResponseDTO(txn_id=kaspiParams.txn_id, sum=order.price_with_taxes,prv_txn_id = kaspiParams.account,
                                                    result=KaspiPaymentStatus.ALREADY_PAID,
                                                    comment="Заказ уже оплачен")

            check_payment = await self.get_first_with_filters(filters=[{"is_failed":False},{"account":kaspiParams.account},{"command":KaspiPaymentCodeStatus.CHECK}])
            if not check_payment:
                return KaspiPaymentPayResponseDTO(txn_id=kaspiParams.txn_id, sum=0, result=KaspiPaymentStatus.NOT_FOUND,prv_txn_id = kaspiParams.account,
                                                  comment="Заказ не найден")
            ...
            check_payment_dto.paid_at = datetime.datetime.strptime(kaspiParams.txn_date, "%Y%m%d%H%M%S")
            ...
            return KaspiPaymentPayResponseDTO(txn_id=paid_order.txn_id, sum=0, result=KaspiPaymentStatus.AVAILABLE_FOR_PAYMENT,prv_txn_id = kaspiParams.account,
