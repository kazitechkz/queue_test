...
from app.core.auth_core import check_admin, check_legal_client
...
from app.feature.organization_employee.dtos.organization_employee_dto import (
    ...
    OrganizationEmployeeRDTO,
    OrganizationEmployeeRDTOWithRelations,
)
from app.feature.organization_employee.filter.organization_employee_filter import (
...
class OrganizationEmployeeController:
    ...
    async def all(
        ...
        result = await repo.paginate_with_filter(
            dto=OrganizationEmployeeRDTOWithRelations,
            page=params.page,
            per_page=params.per_page,
            filters=params.apply(),
            options=[
                selectinload(OrganizationEmployeeModel.employee),
                selectinload(OrganizationEmployeeModel.organization),
            ],
        )
        return result
    ...
    async def my_drivers(
        ...
        organization_id: int = Path(
            gt=0, description="Уникальный идентификатор организации"
        ),
        ...
        if organization_id not in org_ids:
            msg = "Вы не имеете доступа к данной организации"
    ...
    async def create(
        ...
        await self.check_form(
            dto=organization_employee_dto,
            repo=repo,
            userRepo=userRepo,
            organizationRepo=organizationRepository,
        )
        result = await repo.create(
            OrganizationEmployeeModel(**organization_employee_dto.dict())
        )
        return result
    ...
    async def update(
        ...
        result = await repo.update(
            obj=organization_employee, dto=organization_employee_dto
        )
        return result
    ...
    async def check_form(
        ...
        existed_relation = await repo.get_first_with_filters(
            filters=[
        ...
        if existed_relation is not None:
            if existed_relation.id != id:
                raise AppExceptionResponse.bad_request(
                    message="Работник уже прикреплен к организации"
                )

        employee = await userRepo.get_first_with_filters(
            filters=[
                {"id": dto.employee_id},
                {"type_id": TableConstantsNames.UserIndividualTypeId},
            ]
        )
        if employee is None:
