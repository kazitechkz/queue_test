...
from app.feature.operation.dtos.operation_dto import (
    OperationCDTO,
    OperationRDTO,
    OperationWithRelationRDTO,
)
...
class OperationController:
    ...
    async def get_all(
        ...
        result_dto = [
            OperationWithRelationRDTO.from_orm(resultItem) for resultItem in result
        ]
        return result_dto
    ...
    async def check_form(
        ...
        if dto.prev_id is not None:
            ...
            if existed_prev is None:
                raise AppExceptionResponse.bad_request(
                    message="Предыдущего этапа не существует"
                )
            if existed_prev.id == id:
                raise AppExceptionResponse.bad_request(
                    message="Предыдущий этап не может ссылаться на самого себя"
                )
        if dto.next_id is not None:
            existed_next = await repo.get_filtered({"next_id": dto.next_id})
            if existed_next is None:
                raise AppExceptionResponse.bad_request(
                    message="Следующего этапа не существует"
                )
            if existed_next.id == id:
                raise AppExceptionResponse.bad_request(
                    message="Следующий этап не может ссылаться на самого себя"
                )
