...
from sqlalchemy import Computed, Date, ForeignKey, String, Text
...
class VehicleModel(Base):
    ...
    id: Mapped[ID]
    document_number: Mapped[str] = mapped_column(String(length=TableConstantsNames.STANDARD_LENGTH_STRING), unique=True)
    registration_number: Mapped[str] = mapped_column(String(length=TableConstantsNames.STANDARD_LENGTH_STRING), unique=True)
    car_model: Mapped[str] = mapped_column(String(length=TableConstantsNames.STANDARD_LENGTH_STRING))
    ...
    load_max_kg: Mapped[int] = mapped_column(
        Computed("weight_load_max_kg - weight_clean_kg")
    )
    ...
    category_id: Mapped[int] = mapped_column(
        ForeignKey(
            AppTableNames.VehicleCategoryTableName + ".id",
            ondelete="set null",
            onupdate="cascade",
        ),
        nullable=True,
    ...
    color_id: Mapped[int] = mapped_column(
        ForeignKey(
            AppTableNames.VehicleColorTableName + ".id",
            ondelete="set null",
            onupdate="cascade",
        ),
        nullable=True,
    ...
    region_id: Mapped[int] = mapped_column(
        ForeignKey(
            AppTableNames.RegionTableName + ".id", ondelete="set null", onupdate="cascade"
        ),
        nullable=True,
    )
    owner_id: Mapped[int] = mapped_column(
        ForeignKey(
            AppTableNames.UserTableName + ".id", ondelete="set null", onupdate="cascade"
        ),
        nullable=True,
    )
    ...
    category: Mapped["VehicleCategoryModel"] = relationship(back_populates="vehicles")

    color: Mapped["VehicleColorModel"] = relationship(back_populates="vehicles")

    region: Mapped["RegionModel"] = relationship(back_populates="vehicles")

    owner: Mapped["UserModel"] = relationship(back_populates="vehicles")

    organization: Mapped["OrganizationModel"] = relationship(back_populates="vehicles")
