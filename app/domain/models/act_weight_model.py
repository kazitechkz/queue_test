...
class ActWeightModel(Base):
    ...
    vehicle_netto_kg: Mapped[int] = mapped_column(
        Computed("vehicle_brutto_kg - vehicle_tara_kg")
    )
    ...
    history: Mapped["ScheduleHistoryModel"] = relationship(
        "ScheduleHistoryModel", back_populates="act_weights", foreign_keys=[history_id]
    )

    order: Mapped["OrderModel"] = relationship(
        "OrderModel", back_populates="act_weights", foreign_keys=[order_id]
    )

    responsible: Mapped["UserModel"] = relationship(
        "UserModel", back_populates="act_weights", foreign_keys=[responsible_id]
    )

    vehicle: Mapped["VehicleModel"] = relationship(
        "VehicleModel", back_populates="act_weights_vehicle", foreign_keys=[vehicle_id]
    )

    trailer: Mapped["VehicleModel"] = relationship(
        "VehicleModel", back_populates="act_weights_trailer", foreign_keys=[trailer_id]
    )
